...
def training_loop(
    ...
    if rank == 0:
        ...
        img = misc.print_module_summary(G, [z, None])
    ...
    if rank == 0:
        ...
        images = torch.cat([G_ema(z, None).cpu() for z in grid_z]).numpy()
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            D_img, _ = next(training_set_iterator)
            D_z = torch.randn([batch_size, G.z_dim], device=device)
            
            G_img, _ = next(training_set_iterator)
